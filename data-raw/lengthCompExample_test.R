...
dt$`2015` <- rmultinom(1, 50, sim@pLCatch[,1])
y1<-c(rep(sim@LMids, dt$`2015`), rep(NA, (483-50)))
...
MyPars@SL95 <- LifeHistoryExample@L50*1.15
sim<-LBSPRsim(MyPars)
dt$`2016` <- rmultinom(1, 73, sim@pLCatch[,1])
y2<-c(rep(sim@LMids, dt$`2016`), rep(NA, (483-73)))
...
MyPars@SL95 <- LifeHistoryExample@L50*1.15
sim<-LBSPRsim(MyPars)
dt$`2017` <- rmultinom(1, 22, sim@pLCatch[,1])
y3<-c(rep(sim@LMids, dt$`2017`), rep(NA, (483-22)))
...
sim<-LBSPRsim(MyPars)
dt$`2018` <- rmultinom(1, 34, sim@pLCatch[,1])
y4<-c(rep(sim@LMids, dt$`2018`), rep(NA, (483-34)))
...
sim<-LBSPRsim(MyPars)
dt$`2019` <- rmultinom(1, 81, sim@pLCatch[,1])
...
write.csv(dt,file = "data-raw/subset_data.csv",row.names = F)

...
- **`SizeBins`:** List defining length bins for analysis. Can be built automatically using `make_size_bins()` (see Helper functions section below).
...
The `SizeBins` list defines the length bins used in the model. It can be built 
automatically using `make_size_bins()`:
...
- `Linc`: Bin width in cm (default = 1)
- `ToSize`: Upper bound for length classes (automatically calculated as 
`Linf + CVLinf × Linf × MaxSD`)

#### Parameter extraction and setup

The function extracts biological parameters from the S4 life history object `lifeHistoryObj`, including:
...
The model supports both mesh-aggregated and single-mesh selectivity for dome-shaped 
curves.
...
These utility functions simplify the workflow for users and work transparently 
with both `dataType = "Frequency"` and `dataType = "Length"` data.
...
(`CVLinf = 0.1`) and the range multiplier (`MaxSD = 2`).
...
The function returns a list with two elements: `Linc` (bin width) and `ToSize` 
(`Linf + CVLinf × Linf × MaxSD`).
...
| `SizeBins` | A list with `Linc` and `ToSize` elements (from `make_size_bins()`) |


The function returns a data frame with two columns: `Length` (bin midpoints) and 
`Observed` (frequency counts). For `"Frequency"` data, it sums across all catch 
columns. For `"Length"` data, it bins individual measurements into a frequency 
histogram using the model length bins.

#### `plot_fit_pooled()`

The `plot_fit_pooled()` function creates a bar and line plot comparing the observed 
catch length distribution (grey bars) to the model-predicted catch (red line) from 
the pooled GTG dome LBSPR fit. The plot subtitle automatically displays the 
estimated F/M and SPR values.
...
| Argument | Description |
| -------- | ----------- |
| `results_all` | Output from `run_grouped_and_pooled()` |
...
The function works for both `"Frequency"` and `"Length"` data types by calling 
`get_observed_freq()` internally.

#### `plot_fit_by_group()`

The `plot_fit_by_group()` function creates a faceted plot showing observed vs 
predicted catch for each group individually. Each panel displays the group name 
along with its estimated F/M and SPR values. If only one group is present, the 
function skips plotting and prints an informative message.
...
Like `plot_fit_pooled()`, this function handles both `"Frequency"` and `"Length"` 
data types transparently. If only one group is present the function automatically 
skips the plot and prints: `"Only one group present — by-group plot skipped."`
